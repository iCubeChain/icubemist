...
class EthereumNode extends EventEmitter {
    ...
    */
    ...
    getLog() {
        return Settings.loadUserData('icubenode.log');
    }
    ...
    _start(nodeType, network, syncMode) {
        ...
        if (isTestNet) {
            log.info('Node will connect to the test network');
        }
    ...
    __startProcess(nodeType, network, binPath, _syncMode) {
        ...
        return new Q((resolve, reject) => {
            ...
            // rotate the log file
            logRotate(Settings.constructUserDataPath('icubenode.log'), { count: 5 }, (err) => {
                if (err) {
                ...
                */
                ...
                proc.stderr.pipe(
                    fs.createWriteStream(Settings.constructUserDataPath('icubenode.log'), { flags: 'a' })
                );
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'geth') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`iCubeNode error: ${dataStr}`);
