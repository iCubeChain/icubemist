...
//const BINARY_URL = 'http://47.93.9.33/download/clientBinaries.json';
const BINARY_URL = 'https://raw.githubusercontent.com/iCubeChain/icubebinary/master/clientBinaries.json';
...
class Manager extends EventEmitter {
    ...
    _checkForNewConfig(restart) {
        ...
        .then((localConfig) => {
            ...
            return mgr.init({
                folders: [
                    path.join(Settings.userDataPath, 'clientbinaries', 'Geth', 'unpacked'),
                    path.join(Settings.userDataPath, 'clientbinaries', 'Eth', 'unpacked'),
                ],
            })
            .then(() => {
                ...
                if (!available.length) {
                    ...
                    return Q.map(_.values(clients), (c) => {
                        ...
                        return mgr.download(c.id, {
                            downloadFolder: path.join(Settings.userDataPath, 'clientbinaries'),
