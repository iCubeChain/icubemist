...
let menuTempl = function (webviews) {
                        ...
                        click() {
                            ...
                            if (ethereumNode.isEth) {
                                if (process.platform === 'win32') {
                                    userPath = `${Settings.appDataPath}\\iCubeWeb3\\keys`;
                                } else {
                                    userPath += '/.icubeweb3/keys';
                                }

                            // geth
                            } else {
                                if (process.platform === 'darwin') {
                                    userPath += '/Library/iCube/keystore';
                                }

                                if (process.platform === 'freebsd' ||
                                process.platform === 'linux' ||
                                process.platform === 'sunos') {
                                    userPath += '/.icube/keystore';
                                }

                                if (process.platform === 'win32') {
                                    userPath = `${Settings.appDataPath}\\iCube\\keystore`;
                                }
                            }
    ...
    devToolsMenu.push({
        ...
        click() {
            try {
                shell.showItemInFolder(`${Settings.userDataPath}/icubenode.log`);
            } catch (e) {
                log.info(e);
            }
        },
