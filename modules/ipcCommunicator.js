            ...
            if (type === 'ethersale') {
                ...
                if (ethereumNode.isEth) {
                    if (process.platform === 'win32') {
                        keystorePath = `${Settings.appDataPath}\\iCubeWeb3\\keys`;
                    } else {
                        keystorePath += '/.icubeweb3/keys';
                    }
                // geth
                } else {
                    if (process.platform === 'darwin') keystorePath += '/Library/iCube/keystore';

                    if (process.platform === 'freebsd' ||
                        process.platform === 'linux' ||
                        process.platform === 'sunos') keystorePath += '/.icube/keystore';

                    if (process.platform === 'win32') keystorePath = `${Settings.appDataPath}\\iCube\\keystore`;
                }
